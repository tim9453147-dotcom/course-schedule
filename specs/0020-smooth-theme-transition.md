...
- 把 `watch(theme, () => apply())` 改為 watch **字串值鍵**
  `` () => `${theme.value.season}|${theme.value.daypart}` ``，回呼改叫 `applyWithTransition()`。
  （用字串而非陣列：Vue 以 `Object.is` 比較，內容不變不觸發；陣列每次都是新參考會誤觸發，等於沒修好每分鐘 tick 的問題。
  鍵用 `season|daypart` 而非 `primary|neutral|mode`：同季內 dawn↔day、dusk↔night 只有背景漸層改變、
  後三者不變，若用後三者當鍵會漏掉這兩種切換而不過渡。）
