...
export default defineEventHandler(async (event) => {
  ...
  const prompt = [
    ...
    '',
    '辨識規則：',
    '1. 只擷取「課程」內容，全部都當成課程。圖片裡的值星小組、佈置說明、頁首頁尾、裝飾等非課程資訊一律忽略。',
    '2.「DST」也是一堂課，必須列出（不要當成非課程資訊忽略）。',
    '3. host / sharer / summarizer / pm 這四個欄位（即值星小組）一律留空字串，不要填入任何人名。',
    `4. date（日期）：若圖片只有月/日沒有年份，年份一律用 ${year}。`,
    '5. startTime（開始時間）：',
    '   - 課程本身有寫時間，就用寫的時間。',
    '   - 沒寫時間時：課名是「DST」→ 預設 18:30；其餘「週一到週五」的課 → 預設 19:30。',
    '   - 週六、週日且沒寫時間、或無法判斷的，留空字串。',
    '6. endTime（結束時間）：有寫就用寫的，沒寫就留空字串（不要自行推算）。',
    '7. location（地點）：依課表的標題／表頭判斷；個別課程沒另外標明地點時就留空字串。',
    '8. note（備註）：除非課表明確寫了備註，否則留空字串。',
    '',
